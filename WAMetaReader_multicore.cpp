...
void readFiles(std::string folderLocation, int startnum, int endnum, int locator, int max, int cores)
{
	...
	for (const auto& entry : std::experimental::filesystem::recursive_directory_iterator(folderLocation))
	{
		if (tot > startnum && tot < endnum)
		{
			...
			//Places data in the array into the string
			for (int i = 0; i < 26; i++)
			{
				exp += arr[0][i];
				exp += ",";
		...
		if (locator == cores - 1 && multiFolder == false)
		{
			std::cout << "On file: " << totG << " of " << max << "\r";
		}
	}
...
int main(int argc, char* argv[])
{
	...
	//Finds the total number of files so it can divide them amongst the cores(For single folder only)
	if (argc <= 4)
	{
		for (const auto& entry : std::experimental::filesystem::recursive_directory_iterator(folderLocation))
		{
	...
	std::string exp = "";
	std::string arr[3][30];
	//Makes the labels for the csv file(top row)
	arr[0][0] = "file_name";
	for (int i = 1; i < 26; i++)
	{
		arr[0][i] = getID(i);
	...
	exp += "\n";
	//If there are multiple folders to read
	if (argc > 4)
	{
		//Need total to get acurate export location on final[]
		int total = 0;
		//Runs through a loop of each folder location and splits the files in each among the cores. 
		for (int x = 0; x < argc - 3; x++)
		{
			...
			join_all(threadArr);
			threadArr.clear();
			std::cout << "On folder: " << x << " of " << argc - 3 << "\r";
		}
	}
	else
	{
